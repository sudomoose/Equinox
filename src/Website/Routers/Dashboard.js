	...
	setupRoutes() {
		...
		this.router.post('/:id', this.auth, this.getDevelopers.bind(this), this.serverAuth.bind(this), (req, res) => {
			...
			if (
				...
				(req.body['banLog.enabled'] !== 'true' && req.body['banLog.enabled'] !== 'false')
			...
			if (!guild.channels.has(req.body['banLog.channelID'])) return res.status(400).render('error.pug', {
				code: 400,
				message: 'That channel does not exist for the leave message.'
			});
			updateSettings(this.r, guild.id, {
				...
				banLog: {
					enabled: req.body['banLog.enabled'] === 'true',
					channelID: req.body['banLog.channelID']
				},
				kickLog: {
					enabled: req.body['kickLog.enabled'] === 'true',
					channelID: req.body['kickLog.channelID']
				}
			}).then(() => {
